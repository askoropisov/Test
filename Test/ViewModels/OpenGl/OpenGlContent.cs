...
namespace Test.ViewModels.OpenGl
{
...
");
        ...
        {
            var name = typeof(MainView).Assembly.GetManifestResourceNames().First(x => x.Contains("teapot.bin"));
            using (var sr = new BinaryReader(typeof(MainView).Assembly.GetManifestResourceStream(name)!))
            {
        ...
        public unsafe void Init(GlInterface GL, GlVersion version)
        {
            ...
            Info = $"Renderer: {GL.GetString(0x1F01)} Version: {GL.GetString(0x1F02)}";

            // Load the source of the vertex shader and compile it.
            _vertexShader = GL.CreateShader(0x8B31);
            Console.WriteLine(GL.CompileShaderAndGetError(_vertexShader, VertexShaderSource));

            // Load the source of the fragment shader and compile it.
            _fragmentShader = GL.CreateShader(0x8B30);
            ...
            GL.BindBuffer(0x8892, _vertexBufferObject);
            CheckError(GL);
            var vertexSize = Marshal.SizeOf<Vertex>();
            fixed (void* pdata = _points)
                GL.BufferData(0x8892, new IntPtr(_points.Length * vertexSize),
                    new IntPtr(pdata), 0x88E4);

            _indexBufferObject = GL.GenBuffer();
            GL.BindBuffer(0x8893, _indexBufferObject);
            CheckError(GL);
            fixed (void* pdata = _indices)
                GL.BufferData(0x8893, new IntPtr(_indices.Length * sizeof(ushort)), new IntPtr(pdata),
                    0x88E4);
            CheckError(GL);
            _vertexArrayObject = GL.GenVertexArray();
            GL.BindVertexArray(_vertexArrayObject);
            CheckError(GL);
            GL.VertexAttribPointer(positionLocation, 3, 0x1406,
                0, vertexSize, IntPtr.Zero);
            GL.VertexAttribPointer(normalLocation, 3, 0x1406,
            ...
            float yaw, float pitch, float roll, float disco)
        {
            ...
            CheckError(GL);
            GL.DrawElements(0x0004, _indices.Length, 0x1403, IntPtr.Zero);
